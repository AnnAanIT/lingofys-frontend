    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
       ...
       <div className="flex bg-slate-100 p-1 rounded-xl w-fit">
           {(['UPCOMING', 'COMPLETED', 'CANCELLED'] as Tab[]).map(tab => (
               ...
               >
                   {tab === 'UPCOMING' ? 'Upcoming' : tab === 'COMPLETED' ? 'History' : 'Cancelled'}
               </button>
           ))}
       </div>
       ...
       <div className="space-y-4">
           {filteredBookings.length === 0 ? (
               <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center text-slate-500">
                   ...
                   <h3 className="text-lg font-medium text-slate-900">No {activeTab.toLowerCase()} bookings</h3>
                   <p className="mb-6 opacity-70">Your scheduled lessons will appear here.</p>
                   {activeTab === 'UPCOMING' && (
                        <button onClick={() => navigate('/mentee/find-mentor')} className="text-brand-600 font-bold hover:underline">Find a Mentor</button>
                   )}
               </div>
           ) : (
